...
vector < vector <long double> > inverse(vector < vector <long double> > a){ //функция для нахождения обратной матрицы
    int  n = a.size();
...
int main() {
    ...
    for (i=0; i<l; i++){
        ...
        ogirz[i]=-x;
        fl>>x>>y>>z;
        magx[i]=(x-(-0.023323))*5.349270+(y-(-0.438206))*0.178976+(z-(0.066518))*0.066518; //откалиброванные данные=матрица калибровки*(данные-{вектор смещения})
        ...
        a=atan(-aksx[0]/aksy[0]); //углы поворотов в матрице перехода к новой СК (последовательно вокруг Z,X,Z), чтобы g0={0,0,g}, B0={B1,0,B2}
    ...
    for (i=0; i<l-1; i++){
        wx[i]=0;//-h*(-0.0679264*xp2[i]+0.125511*xp3[i]); //вектор шума гироскопа ={0.108275463, 0.125510985, 0.067926358}
        wy[i]=0;//-h*(0.0679264*xp1[i]-0.108275*xp3[i]);  //вектор w-погрешность от гироскопа
        wz[i]=0;//-h*(-0.125511*xp1[i]+0.108275*xp2[i]);
        xm1[i+1]=xp1[i]-h*girz[i]*xp2[i]+h*giry[i]*xp3[i];//xm-вектор, который должен получиться, исходя из данных гироскопа
        ...
        q11[i]=0.0320906*h*h*(xp2[i]*xp2[i]+xp3[i]*xp3[i]); //q-матрица ковариаций процесса
        ...
        pm11[i+1]=p11[i] - h*girz[i]*p21[i] + h*giry[i]*p31[i] - h*girz[i]*(p12[i] - h*girz[i]*p22[i] + h*giry[i]*p32[i]) + h*giry[i]*(p13[i] - h*girz[i]*p23[i] + h*giry[i]*p33[i]) + q11[i]; //матрица pm-промежуточная для вычисления коэффициента калмана
        ...
        mat[0][0]=pm11[i+1]+0.00179455; //0.00179455-норма в квадрате вектора шума акселерометра
        mat[0][1]=pm12[i+1];
        mat[0][2]=pm13[i+1];
        mat[1][0]=pm21[i+1];
        ...
        mat=inverse(mat);
        k11[i+1]=pm11[i+1]*mat[0][0]+pm12[i+1]*mat[1][0]+pm13[i+1]*mat[2][0]; //матрица коэффициентов калмана
        ...
        xp1[i+1]=k11[1 + i]*(naksx[1 + i] - xm1[1 + i]) + xm1[1 + i] + k12[1 + i]*(naksy[1 + i] - xm2[1 + i]) + k13[1 + i]*(naksz[1 + i] - xm3[1 + i]); //xp-профильтрованный вектор
        ...
        xp2[i+1]=xp2[i+1]/e;
        xp3[i+1]=xp3[i+1]/e;
        gamma[i] = atan2(xp2[i],xp3[i]); //крен и тангаж
    ...
    for (i=0; i<l-1; i++){
        ...
        mat[0][0]=pm11[i+1]+0.000107364; //0.00179455-норма в квадрате вектора шума акселерометра
        mat[0][1]=pm12[i+1];
        mat[0][2]=pm13[i+1];
        mat[1][0]=pm21[i+1];
        ...
        xp1[i+1]=k11[1 + i]*(X[1 + i] - xm1[1 + i]) + xm1[1 + i] + k12[1 + i]*(Y[1 + i] - xm2[1 + i]) + k13[1 + i]*(Z[1 + i] - xm3[1 + i]); //xp-профильтрованный вектор
